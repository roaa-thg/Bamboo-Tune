...
**LLM-as-a-Judge Evaluation:** Benchmarks the base and fine-tuned models on a held-out test set using **GPT-4o-mini** as an automated judge, scoring every answer across seven quality dimensions: helpfulness, accuracy, policy alignment, tone, safety, escalation, and conciseness.
...
The evaluation script scored the base and fine-tuned models on the test set with the GPT-4o-mini judge across all seven dimensions. The fine-tuned model improved on **every** dimension, with the largest gains in **escalation**, **safety**, and **conciseness** — exactly the behaviors that matter most for safe, on-label customer support.
